...
- **工作区**：`super+<1-9>` 选择对应的全局编号工作区，`super+0` 选择工作区 10。
  ...
  输入和键盘焦点会立即转到目标工作区。
  `super+shift+<数字>` 则移动聚焦窗口，并在目标工作区打开模态摆放轮廓。方向键或
  标准 Vim H/J/K/L 可组合成八方向，相反方向实时抵消；全部方向键同时松开 110 ms
  后确认。Enter 以当前物理显示器中心确认，Escape 恢复原工作区与精确原位置。
  鼠标移动时预览以指针为窗口中心的精确位置；进入显示器八个有界边/角区域时改为
  对应相对方向，左键确认、右键取消。中心位置从不裁切，因此大窗口允许超出屏幕边缘。
...
Super+Shift+1…9/0 把聚焦窗口移到工作区并摆放、
