...
逐步控制；UI-7E 现只让明确选择该行为的 Settings 导航成为首个视觉接入者，普通列表仍
保持原有标量选中方式。

所有者确认的左侧导航消费者，明确不能实现成在各行之间平移的普通圆角选中框。选中
...
`List::conserved_fluid_selection` 已按此契约接入
`ListTransfer/settings.navigation/page-selection` scope。retained state 持有
`SelectionMassMotion`，paint 从归一化采样生成附着式胶体颈部、移动的 clay/glass 核心与
有界逐行折射；这些变换只发生在绘制层，不会移动布局、命中区或焦点。Settings 的
`UiRoot` 绑定与组合界面相同的 `ThemeRuntime`，所以专业模式写入的 component/transition
曲线和最终无障碍策略会在组件边界生效；运行中切到 Reduced/Off 会走显式 settle 边界，
不会残留空间动画。
...
初值覆盖。选中关键帧数值绑定已经接通；预设快照、导入/导出与恢复也已组合成检查器内的
预设模式。选择修订只替换曲线图与真实预览的渲染来源，不能修改实时编辑文档；草稿未保存
时，应用预设必须明确放弃，或先成功保存不可变草稿快照。用户修订支持搜索、分组历史、
删除和剪贴板导入/导出，内置预设保持只读。同一个宿主 worker 串行执行写入，Config1
signal watcher 可在 worker reply 前确认写入，也会无轮询跟随合法外部 profile/资料库变化。
紧凑与展开的左侧导航现已启用上述守恒液态选中；连续输入只保留最新目标，并从当前质量
分布和切线继续，不会排队或重播固定 clip。
